(() => {
  ...
  function renderDetail(item) {
    ...
    detailNode.querySelectorAll(".thumbnails button").forEach((button) => button.addEventListener("click", () => { mainImage.src = button.dataset.image; detailNode.querySelectorAll(".thumbnails button").forEach((entry) => entry.classList.toggle("selected", entry === button)); }));
    ...
    detailNode.querySelector(".share-product").addEventListener("click", async () => { const url = location.href; try { if (navigator.share) await navigator.share({ title:item.title, url }); else await navigator.clipboard.writeText(url); } catch {} });
  }
  function render() {
    if (!catalog) return;
